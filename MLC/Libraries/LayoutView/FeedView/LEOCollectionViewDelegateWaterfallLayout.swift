...
public class LEOCollectionViewWaterfallLayout: UICollectionViewLayout {
    ...
    public func minimumLineSpacingForSection (_ section: Int) -> CGFloat {
        if let miniumSpaceing = self.delegate?.leo_collectionView?(self.collectionView!, layout: self, minimumLineSpacingForSectionAtIndex: section) {
            return miniumSpaceing
        } else {
            return self.minimumLineSpacing
        }
    }

    public func minimumColumnSpacingForSection (_ section: Int) -> CGFloat {
        if let minimumColumnSpacing = self.delegate?.leo_collectionView?(self.collectionView!, layout: self, minimumColumnSpacingForSectionAtIndex: section) {
    ...
    public func itemWidthInSectionAtIndex (_ section: Int) -> CGFloat {
        ...
        let minimumColumn = self.minimumColumnSpacingForSection(section)
        return floor((width - (spaceColumCount * minimumColumn)) / CGFloat(columnCount))
    }
    ...
    override public func prepare() {
        ...
        for section in 0 ..< numberOfSections {
            ...
            if let miniumSpaceing = self.delegate?.leo_collectionView?(self.collectionView!, layout: self, minimumLineSpacingForSectionAtIndex: section) {
                minimumLineSpacing = miniumSpaceing
            } else {
            ...
            let minimumColumn = self.minimumColumnSpacingForSection(section)
            let itemWidth = floor((width - (spaceColumCount * minimumColumn)) / CGFloat(columnCount))
            ...
            for idx in 0 ..< itemCount {
                ...
                let xOffset = sectionInsets.left + (itemWidth + minimumColumn) * CGFloat(columnIndex)
                ...
                self.columnHeights[section][columnIndex] = attributes.frame.maxY + minimumLineSpacing
            ...
            top = self.columnHeights[section][columnIndex] - minimumLineSpacing + sectionInsets.bottom
